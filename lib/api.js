...
function createRequest(resource, params) {
    ...
    const url = `https://${config.subDomain}.nokotime.com/api/${resource}${urlParams}`;

    return {
        url,
        headers: {
            'X-NokoToken': config.apiToken
        }
    };
}
...
function parseEntries(nokoEntries) {
    return nokoEntries.map(nokoEntry => {

        const date = moment(nokoEntry.entry.date);
        const duration = nokoEntry.entry.minutes * 60 * 1000;
