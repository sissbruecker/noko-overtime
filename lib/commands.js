...
const debug = require('debug')('noko');
...
function configure(options) {
    return inquirer
        .prompt([
            {
                type: 'input',
                message: 'Enter your Noko sub-domain',
                name: 'subDomain',
                validate(input) {
            ...
            {
                type: 'password',
                message: 'Enter your Noko API authentication token',
                name: 'apiToken',
                validate(input) {
                    return input && input.length
                        ? true
...
async function update() {
    ...
    try {
        ...
        debug(error);
        spinner.fail('Sorry, I could not load your time entries from Noko. Please make sure you have configured the correct sub-domain and API token.');
        return;
    }
